...
def returnFolderFirstLevel():
    ...
    with open('./foo.txt', 'w') as fp:
        ...
        for i in range(countArray.__len__()):
            ...
            print(i)
            fp.write(str(data[0][i])+'\t' + str(float(countArray[i])/2)+'\t'+str(float(countArray[i])*400/2)+'\n')
        # fp.write(str('summ'+'\t'+str(allscene)+'\t'+str(allarea)))    \\\\\\ строка для отображения суммы площади
